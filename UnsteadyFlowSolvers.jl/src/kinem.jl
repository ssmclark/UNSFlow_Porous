...
struct AlphaDragonDef <: MotionDef
    Tperiod :: Float64
    phase :: Float64
end
  
function (alp::AlphaDragonDef)(t)
    -(pi/4)+(pi/4)*sin(2*pi*(t)/alp.Tperiod+alp.phase)
end

struct UDragonDef <: MotionDef
    Tperiod :: Float64
    phase :: Float64
    amp :: Float64
    incident :: Float64
...
function (kin::UDragonDef)(t)
    -(kin.amp)*pi*cos(kin.incident)/(kin.Tperiod)*sin((2*pi*(t)/kin.Tperiod)+kin.phase)
    #(kin.amp)*cos(kin.incident)/2*(cos((2*pi*(t)/(kin.Tperiod))+(kin.phase))+1)
end


struct hDragonDef <: MotionDef
    Tperiod :: Float64
    phase :: Float64
    amp :: Float64
    incident :: Float64
...
function (kin::hDragonDef)(t)
    (kin.amp)*sin(kin.incident)/2*(cos((2*pi*(t)/(kin.Tperiod))+(kin.phase))+1)
end




#Dragonfly angle attack calculations
struct DragonDef <: MotionDef
    Upstroke :: Float64
    Dwstroke :: Float64
    k :: Float64
    deltaT :: Float64
  ...
  function (kin::DragonDef)(t)
    Meanpos=((kin.Upstroke)+(kin.Dwstroke))/2
    MeanNeg=((kin.Upstroke)-(kin.Dwstroke))/2
    T=(t/(kin.k))-floor(t/kin.k);
    if((T>=(-kin.deltaT/2)) && (T<(kin.deltaT/2)))
        amp=(Meanpos)+((MeanNeg)*sin(pi*(T/kin.deltaT)));
    elseif((T>=(kin.deltaT/2)) && (T<((1-kin.deltaT)/2)))
        amp=(kin.Upstroke)

    elseif((T>=((1-kin.deltaT)/2)) && (T<((1+kin.deltaT)/2)))
        amp=(Meanpos)+((MeanNeg)*sin(pi*(((1/2)-T)/kin.deltaT)));
    ...
    elseif(((T>=((1+kin.deltaT)/2))) && (T<1-(kin.deltaT/2))) 
        ...
        T=T-1;
        amp=(Meanpos)+((MeanNeg)*sin(pi*(T/kin.deltaT)));
        println("working in DragonDef5")
    end
    amp
  end

  struct positionDragonDef <: MotionDef
    k :: Float64
  end
  
  function (kin::positionDragonDef)(t)
    T=((t)/(kin.k))-floor((t)/kin.k);
    (5*pi/180)+(30*pi/180*cos(2*pi*T))
  end
#Dragonfly Vortex angle attack calculations
#Alpha

#dragon def velocity
struct DragonDefVel <: MotionDef
    Upstroke :: Float64
    Dwstroke :: Float64
    k :: Float64
    deltaT :: Float64
  ...
  function (kin::DragonDefVel)(t)
    MeanNeg=((kin.Upstroke)-(kin.Dwstroke))/2
    T=(t/(kin.k))-floor(t/kin.k);
    if((T>=(-kin.deltaT/2)) && (T<(kin.deltaT/2)))
        amp=((MeanNeg)*(pi/kin.deltaT)*sin(pi*(T/kin.deltaT)));
    elseif((T>=(kin.deltaT/2)) && (T<((1-kin.deltaT)/2)))
        amp=0

    elseif((T>=((1-kin.deltaT)/2)) && (T<((1+kin.deltaT)/2)))
        amp=((MeanNeg)*(pi/kin.deltaT)*sin(pi*(((1/2)-T)/kin.deltaT)));
