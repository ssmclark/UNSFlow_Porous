...
"""
function lautat(surf::TwoDSurfPorous, curfield::TwoDFlowField, nsteps::Int64 = 500, dtstar::Float64 = 0.015, startflag = 0, writeflag = 0, writeInterval = 1000., delvort = delNone(); maxwrite = 50, nround=6, wakerollup=1)
        ...
        if nshed > 0
            ...
            for i = 1:nsurf
                if i in shed_ind
                    levcount += 1
                    curfield.tev[end-nsurf+i].s = soln.zero[i]
                    curfield.lev[end-nsurf+levcount].s = soln.zero[nsurf+levcount]
                    surf[i].levflag[1] = 1
                else
                    curfield.tev[end-nsurf+i].s = soln.zero[i]
                    surf[i].levflag[1] = 0 
...
function ldvm2DOF(surf::TwoDSurfPorous, curfield::TwoDFlowField, kinem::KinemPar2DOF, strpar::TwoDOFPar ,nsteps::Int64 = 500, dtstar::Float64 = 0.015, startflag = 0, writeflag = 0, writeInterval = 1000., delvort = delNone(); maxwrite = 50, nround=6)
...
function ldvm2DOFPorous(surf::TwoDSurfPorous, curfield::TwoDFlowField, strpar::TwoDOFPar, kinem::KinemPar2DOF, nsteps::Int64 = 500, dtstar::Float64 = 0.015, startflag = 0, writeflag = 0, writeInterval = 1000., delvort = delNone(); maxwrite = 50, nround=6)
    ...
    #pressure. does it need to be here?
    # p_com = 0.
    cl = 0.
    cm = 0.
    ...
    for istep = 1:nsteps
        ...
        #Update kinematic parameters (based on porous 2DOF response)
        #keep original update_kinem2DOF here for now
        update_kinem2DOF(surf, strpar, kinem, dt, cl, cm)
        ...
        #Add a TEV with dummy strength
        place_tev_porous(surf,curfield,dt)
      ...
      #  p_com = calc_delcp(surf, [curfield.u[1], curfield.w[1]])
...
function ldvmVariation(surf::Vector{TwoDSurf}, curfield::TwoDFlowField, nsteps::Int64 = 500, dtstar::Float64 = 0.015, startflag = 0, writeflag = 0, writeInterval = 1000., nLesp = 30, delvort = delNone(); maxwrite = 50, nround=6)
    ...
    elseif startflag == 1
        ...
        dirresults = map(x->(v = tryparse(Float64,x); typeof(v) == Nothing ? 0.0 : v),dirvec)
        latestTime = maximum(dirresults)
        mat = readdlm("resultsSummary-$nLesp")
        t = mat[end,1]
    else
        throw("invalid start flag, should be 0 or 1")
    end
...
function ldvmVariation(surf::TwoDSurf, curfield::TwoDFlowField, nsteps::Int64 = 500, dtstar::Float64 = 0.015, startflag = 0, writeflag = 0, writeInterval = 1000., nLesp = 30, delvort = delNone(); maxwrite = 50, nround=6)
    ...
    elseif startflag == 1
        ...
        dirresults = map(x->(v = tryparse(Float64,x); typeof(v) == Nothing ? 0.0 : v),dirvec)
        latestTime = maximum(dirresults)
        mat = DelimitedFiles.readdlm("resultsSummary-$nLesp",header = true)
        t = mat[end,1]
    else
        throw("invalid start flag, should be 0 or 1")
    end
